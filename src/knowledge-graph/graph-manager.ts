...
import { SimilarityEngine, RelationshipType } from '../semantic/similarity-engine';
...
interface GraphEdge {
    ...
    target: string;
    weight: number;
    type: 'explicit' | RelationshipType;
    confidence: number;
}
        ...
        for (let i = 0; i < clusters.length; i++) {
            ...
            for (let j = 0; j < clusterNodes.length; j++) {
                for (let k = j + 1; k < clusterNodes.length; k++) {
                    // For performance, use simple cosine similarity of vectors
                    const vectorA = clusterNodes[j].embedding!;
                    const vectorB = clusterNodes[k].embedding!;
    ...
    private async performKMeansClustering(nodes: GraphNode[], k: number): Promise<GraphNode[][]> {
        ...
        for (let iter = 0; iter < 20; iter++) {
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                for (let j = 0; j < k; j++) {
                    // Calculate cosine similarity directly for performance
                    const vectorA = embeddings[i];
                    const vectorB = centroids[j];
                    ...
                    for (let dim = 0; dim < vectorA.length; dim++) {
                        ...
                        normA += vectorA[dim] * vectorA[dim];
                        normB += vectorB[dim] * vectorB[dim];
                    }
                    const similarity = dotProduct / (Math.sqrt(normA) * Math.sqrt(normB));
