...
export class ComprehensiveResearchSystem {
    // Standard research template for reset functionality
    static readonly RESEARCH_STANDARD_TEMPLATE = `---
project: "{{projectName}}"
status: "in-progress"
created: "{{today}}"
updated: "{{today}}"
quality: "pending"
clippy_id: "{{researchId}}"
tags: {{tags}}
---
...
`;
    ...
    private showcaseMode: { enabled: boolean; options?: any } = { enabled: false };
    ...
    async processResearchChecklist(
        ...
        // Check for showcase mode - if enabled, generate mock research instead of real research
        if (this.showcaseMode.enabled) {
            console.log('🎭 Showcase mode enabled - generating demo research notes');
            await this.processShowcaseResearch(checklist, options, onProgress);
            return;
        }
    ...
    private async createBlankResearchNotes(
        ...
        for (const item of checklist) {
            ...
            // Use AI-enhanced template generation for custom templates
            const template = await this.generateAIEnhancedTemplate(item, options);
    ...
    /**
     * Generate comprehensive blank research template using template system.
     */
    private generateBlankResearchTemplate(item: ChecklistItem, options: any = {}): string {
        const today = new Date().toISOString().split('T')[0];
        
        // Use custom template if provided, otherwise use the configured research template
        const templateToUse = options.customTemplate || 
                             this.plugin.settings?.research?.templates?.research || 
                             ComprehensiveResearchSystem.RESEARCH_STANDARD_TEMPLATE;
        
        return this.processCustomTemplate(templateToUse, item, today, options);
    }

    /**
     * Generate research template with AI-powered content filling for custom templates
     */
    private async generateAIEnhancedTemplate(item: ChecklistItem, options: any = {}, extractedWisdom?: ExtractedWisdom): Promise<string> {
        const today = new Date().toISOString().split('T')[0];
        ...
        // If it's a custom template (not one of the built-in ones), use AI enhancement
        if (options.customTemplate && options.customTemplate !== 'research-standard' && 
            options.customTemplate !== 'research-minimal' && options.customTemplate !== 'research-scientific') {
            console.log('🤖 Using AI-enhanced template processing for custom template');
            return await this.processCustomTemplateWithAI(templateToUse, item, options, extractedWisdom);
        } else {
            // Use regular processing for built-in templates
            return this.processCustomTemplate(templateToUse, item, today, options);
        }
    }
     ...
     */
    private processCustomTemplate(template: string, item: ChecklistItem, today: string, options: any = {}): string {
        return template
            .replace(/\{\{title\}\}/g, item.name)
            .replace(/\{\{researchTopic\}\}/g, item.name)
            .replace(/\{\{item\.name\}\}/g, item.name)
            .replace(/\{\{item\.id\}\}/g, item.id)
            .replace(/\{\{researchId\}\}/g, item.id)
            .replace(/\{\{projectName\}\}/g, options.projectName || 'Default Research Project')
...
- [ ] Research completed`)
    ...
    /**
     * Process custom template with AI-powered content generation for headings
     */
    private async processCustomTemplateWithAI(template: string, item: ChecklistItem, options: any = {}, extractedWisdom?: ExtractedWisdom): Promise<string> {
        console.log(`🤖 Processing custom template with AI for: ${item.name}`);
        
        // First do basic variable substitution
        let processedTemplate = this.processCustomTemplate(template, item, new Date().toISOString().split('T')[0], options);
        
        // Extract headings and identify sections that need AI filling
        const headingSections = this.extractHeadingSections(processedTemplate);
        
        if (headingSections.length === 0) {
            console.log('📝 No heading sections found in template');
            return processedTemplate;
        }

        // Generate AI content for each empty section
        for (const section of headingSections) {
            if (this.shouldFillSection(section.content)) {
                try {
                    const aiContent = await this.generateSectionContent(section.heading, item.name, options, extractedWisdom);
                    processedTemplate = processedTemplate.replace(section.content, aiContent);
                    console.log(`✨ Generated AI content for section: ${section.heading}`);
                } catch (error) {
                    console.error(`❌ Failed to generate content for section ${section.heading}:`, error);
                    // Keep the original placeholder if AI generation fails
                }
            }
        }
    ...
    /**
     * Extract heading sections from markdown template
     */
    private extractHeadingSections(template: string): Array<{heading: string, content: string, level: number}> {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (headingMatch) {
                ...
                if (inSection && currentHeading && this.shouldFillSection(currentContent)) {
                    sections.push({
    ...
    /**
     * Check if a section needs AI filling (is empty or has placeholder text)
     */
    private shouldFillSection(content: string): boolean {
    ...
    private async generateSectionContent(
        ...
        topic: string, 
        options: any = {}, 
        extractedWisdom?: ExtractedWisdom
    ): Promise<string> {
        // Reason: Optimize by reusing extracted wisdom data instead of making new AI calls
        if (extractedWisdom) {
            const content = this.mapWisdomToSection(heading, extractedWisdom);
            if (content) {
                console.log(`♻️ Reused wisdom data for section: ${heading}`);
                return content;
            }
        }

        // Fallback to AI generation only if wisdom doesn't contain relevant data
        if (!this.plugin?.aiProvider) {
            throw new Error('AI provider not available');
        }

        const prompt = `You are helping to fill out a research template section. Generate concise, informative content for the following section:

**Topic**: ${topic}
**Section**: ${heading}

Requirements:
- Write 2-4 sentences of clear, factual content
- Focus on the specific aspect indicated by the section heading
- Use markdown formatting where appropriate
- Be informative but concise
- Do not repeat the heading in your response

Generate content for this section:`;

        try {
            console.log(`🤖 Using AI for section: ${heading} (wisdom data not available)`);
            const response = await this.plugin.aiProvider.generateText(prompt, {
                maxTokens: 200,
                temperature: 0.3
            });
        ...
        try {
            ...
            for (let i = 0; i < searchResults.length; i++) {
                ...
                try {
                    ...
                    // Check if file already exists, if so, skip or overwrite
                    let file: TFile;
                    const existingFile = this.app.vault.getAbstractFileByPath(filePath);
                    if (existingFile && existingFile instanceof TFile) {
                        // File already exists, update it instead of creating
                        await this.app.vault.modify(existingFile, noteContent);
                        file = existingFile;
                        console.log(`📝 Updated existing web search note: ${fileName}`);
                    } else {
                        // Create new file
                        file = await this.app.vault.create(filePath, noteContent);
                        console.log(`💾 Created new web search note: ${fileName}`);
                    }
...
`;
    ...
    private async extractWisdomFromAllSources(
        ...
        try {
            ...
            // Reason: Return raw wisdom text instead of parsing into categories
            // This raw text will be reused directly in each heading prompt
            return {
                rawWisdom: cleanedResponse,
                sources: [...vaultNotes.map(n => n.file.basename), ...webSearchNotes.map(w => w.title)]
            };
        } catch (error) {
            console.error('AI wisdom extraction failed:', error);
            return {
                rawWisdom: `Research material extraction failed: ${error.message}`,
                sources: []
            };
        }
    }
...
Focus on accuracy, cite contradictions if found, and prioritize information from multiple sources.`;
    ...
    private async generateResponsesForAllSections(
        ...
        for (const heading of templateStructure.headings) {
            console.log(`♻️ Reusing extracted wisdom for: ${heading.text}`);
            
            // Reason: Use raw wisdom directly in simple prompt format as specified
            // "{{heading}} as it relates to {{researchTopic}} here is the research material {{extracted_wisdom}}"
            const sectionPrompt = `Write a section about "${heading.text}" as it relates to "${searchTerm}". Here is the research material to draw answers from:

${wisdom.rawWisdom}

Provide a clear, informative response for this specific section. Focus only on information relevant to "${heading.text}".`;

            try {
                if (!this.plugin?.aiProvider?.generateText) {
                    console.warn(`AI Provider not available for "${heading.text}"`);
                    responses.set(heading.fullMatch, `AI Provider not available. Please check plugin configuration.`);
                    continue;
                }
                
                const response = await this.plugin.aiProvider.generateText(sectionPrompt, {
                    maxTokens: 300,
                    temperature: 0.3
                });
                responses.set(heading.fullMatch, response.trim());
                console.log(`✅ Generated content for: ${heading.text} using extracted wisdom`);
            } catch (error) {
                console.warn(`Failed to generate content for "${heading.text}":`, error);
                responses.set(heading.fullMatch, `Unable to generate content for ${heading.text} from available research sources.`);
            }
        }
...
**Research Context:**
- Topic: ${searchTerm}
- Key facts: ${wisdom.keyFacts?.slice(0, 3).join('; ') || 'No key facts available'}
- Uses: ${wisdom.uses?.slice(0, 2).join('; ') || 'No uses available'}
...
**Available Data:** ${wisdom.keyFacts?.join('; ') || 'No key facts available'}
...
**Respond with only the value (no quotes unless it's a string that needs them):**`;
    ...
    private parseWisdomResponse(response: string, vaultNotes: VaultNote[], webSearchNotes: WebSearchNote[]): ExtractedWisdom {
        ...
        return {
            rawWisdom: 'Content combined from multiple sources',
            ...sections,
            sources: [
    ...
    private createFallbackWisdom(vaultNotes: VaultNote[], webSearchNotes: WebSearchNote[]): ExtractedWisdom {
        return {
            rawWisdom: 'Fallback wisdom - information compiled from multiple sources',
...
- [x] Wisdom extracted and processed`;
    ...
    private async enhanceFrontmatterWithCentralizedTags(
        ...
        try {
            ...
            // Use 'general' as default research type (no AI needed)
            const researchType = 'general';
    ...
    /**
     * Get showcase mode status
     */
    getShowcaseStatus(): { enabled: boolean; mode?: string } {
        return { 
            enabled: this.showcaseMode.enabled, 
            mode: this.showcaseMode.options?.mode || 'demo'
        };
    }
     ...
     */
    enableShowcaseMode(options?: { mode?: string; templateVariables?: boolean; includeRealData?: boolean; mockDataSample?: string }): void {
        this.showcaseMode = { 
            enabled: true, 
            options: options || {} 
        };
        console.log('🎭 Showcase mode enabled:', options);
        // In showcase mode, the research system could use mock data or enhanced debugging
    }

    /**
     * Process research in showcase mode - generates demo content without API calls
     */
    private async processShowcaseResearch(
        checklist: ChecklistItem[],
        options: any = {},
        onProgress?: (progress: { current: number; total: number; percentage: number; message: string }) => void
    ): Promise<void> {
        console.log('🎭 Processing research in showcase mode');
        ...
        const projectId = await this.projectTracker.createProject(
            projectName,
            '🎭 Showcase: Framework testing with template variables',
            checklist,
            options
        );

        // Initialize progress tracking
        const totalSteps = checklist.length * 3; // Simplified steps for showcase
        let currentStep = 0;
        ...
        updateProgress("🎭 Initializing showcase mode...");

        for (const [index, item] of checklist.entries()) {
            try {
                updateProgress(`🎭 Processing showcase item: ${item.name}`);
                
                // Generate showcase content
                const showcaseContent = this.generateShowcaseContent(item, options);
                
                updateProgress(`🎭 Creating demo note for: ${item.name}`);
                
                // Create the research note with showcase content
                const notePath = await this.createShowcaseNote(
                    item,
                    showcaseContent,
                    options.outputFolder || 'Generated Research Notes'
                );
                
                // Mark item as completed
                await this.projectTracker.markItemCompleted(projectId, item.id, notePath);
                
                updateProgress(`✅ Completed showcase item: ${item.name}`);
                
                // Small delay to simulate processing
                await new Promise(resolve => setTimeout(resolve, 500));
                
            } catch (error) {
                console.error(`❌ Showcase processing failed for ${item.name}:`, error);
                await this.projectTracker.markItemFailed(projectId, item.id, String(error));
            }
        }

        console.log(`🎭 Showcase research complete for ${checklist.length} items`);
        updateProgress(`🎭 Showcase research complete!`);
    }

    /**
     * Generate showcase content for demonstration purposes
     */
    private generateShowcaseContent(item: ChecklistItem, options: any): string {
        const mockDataSample = this.showcaseMode.options?.mockDataSample || "This is demo content generated for framework testing.";
        
        return `# 🔬 ${item.name}

## 📊 Research Status
- [x] Vault notes analyzed  
...
### Vault Analysis (Showcase)
Found relevant information from your existing notes. This showcases how the system would integrate with your vault content.

### Web Research Summary (Showcase)
This section demonstrates the web research integration. In production mode, this would contain actual web search results and AI analysis.

### Key Insights
1. **Primary Concept**: Framework demonstration of research capabilities
2. **Secondary Applications**: Template variable processing 
3. **Integration Points**: Vault content analysis and web research synthesis
...
*🎭 This is a showcase note demonstrating the research framework with template variables. Enable production mode for full AI-powered research.*`;
    }

    /**
     * Create a showcase research note
     */
    private async createShowcaseNote(
        item: ChecklistItem,
        content: string,
        outputFolder: string
    ): Promise<string> {
        try {
            ...
            // Create the note
            const noteTitle = `${item.name}`;
            const sanitizedTitle = noteTitle.replace(/[\\/:*?"<>|]/g, '');
            const notePath = `${outputFolder}/${sanitizedTitle}.md`;
            ...
            if (existingFile) {
                ...
                await this.app.vault.create(notePath, processedContent);
            }
            
            console.log(`🎭 Created showcase note: ${notePath}`);
            return notePath;
            
        } catch (error) {
            console.error(`❌ Failed to create showcase note for ${item.name}:`, error);
            throw error;
        }
    }
     ...
     */
    private processTemplateVariables(content: string, variables: any): string {
    ...
    disableShowcaseMode(): void {
        this.showcaseMode = { enabled: false };
        console.log('🔬 Showcase mode disabled - returning to production mode');
    }
}