...
export class RAGSystem {
  ...
  private createSmartChunks(content: string, documentId: string): RAGChunk[] {
    ...
    // Use configurable chunk parameters from centralized RAG settings
    const maxChunkSize = this.settings?.rag?.chunking?.chunkSize || 1000; // characters
    const overlap = this.settings?.rag?.chunking?.chunkOverlap || 100; // character overlap between chunks
